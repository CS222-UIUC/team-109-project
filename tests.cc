...
TEST_CASE("canSplit returns true for two cards equal value", "[blackjack]") {
    BlackJack game;
...
TEST_CASE("canSplit returns false for two card different values", "[blackjack]") {
    BlackJack game;

    game.playerHand = { {"10", "Spades"}, {"J", "Clubs"} };

    REQUIRE(game.canSplit() == false);
}
